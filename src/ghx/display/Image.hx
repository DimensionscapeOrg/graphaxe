...
class Image extends Quad {
	...
	private static var sBasCols:Vector<Float> = new Vector<Float>(3);
	private static var sBasRows:Vector<Float> = new Vector<Float>(3);
	private static var sPosCols:Vector<Float> = new Vector<Float>(3);
	private static var sPosRows:Vector<Float> = new Vector<Float>(3);
	private static var sTexCols:Vector<Float> = new Vector<Float>(3);
	private static var sTexRows:Vector<Float> = new Vector<Float>(3);

	#if commonjs
	private static function __init__() {
		untyped Object.defineProperties(Image.prototype, {
			"scale9Grid": {
				get: untyped __js__("function () { return this.get_scale9Grid (); }"),
				set: untyped __js__("function (v) { return this.set_scale9Grid (v); }")
			},
			"tileGrid": {
				get: untyped __js__("function () { return this.get_tileGrid (); }"),
				set: untyped __js__("function (v) { return this.set_tileGrid (v); }")
			},
		});
	}
	#end
	...
	private function set_tileGrid(value:Rectangle):Rectangle {
		if (value != null) {
			if (__tileGrid == null)
				__tileGrid = value.clone();
			else
				__tileGrid.copyFrom(value);
	...
	override private function __setupVertices():Void {
		if (texture != null && __tiled9Grid != null)
			__setupTiled9Grid();
		else if (texture != null && __scale9Grid != null)
			__setupScale9Grid();
		else if (texture != null && __tileGrid != null)
			__setupTileGrid();
		else
			super.__setupVertices();
	}

	/** @private */
	override private function set_scaleX(value:Float):Float {
		super.scaleX = value;
		if (texture != null && (__scale9Grid != null || __tileGrid != null || __tiled9Grid != null))
			__setupVertices();
		return value;
	}
	...
	@:noCompletion private function __setupScale9Grid():Void {
		...
		numVertices = __setupScale9GridAttributes(sPadding.left, sPadding.top, sPosCols, sPosRows, sTexCols, sTexRows);
	...
	@:noCompletion private function __setupScale9GridAttributes(startX:Float, startY:Float, posCols:Vector<Float>, posRows:Vector<Float>,
	...
	private var __tiled9Grid:Rectangle;

	public var tiled9Grid(get, set):Rectangle;

	private function get_tiled9Grid():Rectangle {
		return __tiled9Grid;
	}

	private function set_tiled9Grid(value:Rectangle):Rectangle {
		if (value != null) {
			if (__tiled9Grid == null)
				__tiled9Grid = value.clone();
			else
				__tiled9Grid.copyFrom(value);
	...
	private function __setupTiled9Grid():Void {
		var texture:Texture = this.texture;
		var frame:Rectangle = texture.frame;
		var vertexData:VertexData = this.vertexData;
		var indexData:IndexData = this.indexData;
		...
		var frameWidth:Float = __tiled9Grid.width > 0 ? __tiled9Grid.width : texture.frameWidth;
		var frameHeight:Float = __tiled9Grid.height > 0 ? __tiled9Grid.height : texture.frameHeight;
		...
		var tileWidth:Float = __tiled9Grid.width * (frameWidth / texture.frameWidth);
		var tileHeight:Float = __tiled9Grid.height * (frameHeight / texture.frameHeight);
		var modX:Float = (__tiled9Grid.x * invScaleX) % frameWidth;
		var modY:Float = (__tiled9Grid.y * invScaleY) % frameHeight;
	...
	private function __set3Slice(y:Float, height:Float):Void {
		if (texture != null) {
			if (__scale9Grid == null) {
				__scale9Grid = new Rectangle(0, y, texture.width, height);
			} else {
				__scale9Grid.setTo(0, y, texture.width, height);
			}
	...
	public static function automateSetupForTexture(texture:Texture, onAssign:Image->Void, onRelease:Image->Void = null):Void {
		...
		if (automator != null)
			...
			sAutomators[texture] = new SetupAutomator(onAssign, onRelease);
	}
