...
class Stage extends DisplayObjectContainer {
	...
	private static function __init__() {
		untyped Object.defineProperties(Stage.prototype, {
			...
			"stageWidth": {get: untyped __js__("function () { return this.get_stageWidth (); }"),
				set: untyped __js__("function (v) { return this.set_stageWidth (v); }")},
			"stageHeight": {get: untyped __js__("function () { return this.get_stageHeight (); }"),
				set: untyped __js__("function (v) { return this.set_stageHeight (v); }")},
			"starling": {get: untyped __js__("function () { return this.get_starling (); }"),
				set: untyped __js__("function (v) { return this.set_starling (v); }")},
			"focalLength": {get: untyped __js__("function () { return this.get_focalLength (); }"),
				set: untyped __js__("function (v) { return this.set_focalLength (v); }")},
			"fieldOfView": {get: untyped __js__("function () { return this.get_fieldOfView (); }"),
				set: untyped __js__("function (v) { return this.set_fieldOfView (v); }")},
			"projectionOffset": {get: untyped __js__("function () { return this.get_projectionOffset (); }"),
				set: untyped __js__("function (v) { return this.set_projectionOffset (v); }")},
	...
	public override function hitTest(localPoint:Point):DisplayObject {
		...
		if (localPoint.x < 0 || localPoint.x > __width || localPoint.y < 0 || localPoint.y > __height)
			return null;
	...
	public function getCameraPosition(space:DisplayObject = null, out:Vector3D = null):Vector3D {
		...
		return MatrixUtil.transformCoords3D(sMatrix3D, __width / 2 + __projectionOffset.x, __height / 2 + __projectionOffset.y, -focalLength, out);
	}
	...
	@:allow(starling) private function addEnterFrameListener(listener:DisplayObject):Void {
		...
		if (index < 0)
			__enterFrameListeners[__enterFrameListeners.length] = listener;
	}
	...
	@:allow(starling) private function removeEnterFrameListener(listener:DisplayObject):Void {
		var index:Int = __enterFrameListeners.indexOf(listener);
		if (index >= 0)
			__enterFrameListeners.splice(index, 1);
	}

	/** @private */
	@:allow(starling) private override function __getChildEventListeners(object:DisplayObject, eventType:String, listeners:Array<DisplayObject>):Void {
